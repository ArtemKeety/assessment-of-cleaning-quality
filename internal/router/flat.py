...
async def get_flats(user_data = Depends(user_identy), service = Depends(UoWService.from_request)):
    return await service.Flat.all(user_data.get('user_id'))

@router.get('/{flat_id}', response_model=list[FullFlat])
async def get_id(flat_id: int, service = Depends(UoWService.from_request)):
    return await service.Flat.get_id(flat_id)

@router.delete('/{flat_id}',response_model=int, dependencies=[Depends(user_identy)])
async def delete(flat_id: int, service = Depends(UoWService.from_request)):
